using JuMP
using Gurobi
log_file_path = "/scratchbeta/clementf/45_nc.txt"
...
@constraint(Sparse, [i in 1:m, j in 1:m], 1/m* sum(a[u,v] for u in 1:i, v in 1:j) - x[i]*y[j] <= z)

# constraints (5b)
@constraint(Sparse, [i in 1:m+1, j in 1:m+1], -1/m* sum(a[u,v] for u in 1:i-1, v in 1:j-1) + x[i]*y[j] <= z)
...
output_file= open("/scratchbeta/clementf/OC_45_nc.jl", "w")
write(output_file, "x = ")
