using JuMP
using Gurobi
log_file_path = "/scratchbeta/clementf/Pb4.txt"
...
@constraint(Sparse, [i in 1:m, j in 1:m, k in 1:i-1, l in 0:j], 1/m*sum( (sum(a[u,v] for u in 1:k)+sum(a[u,v] for u in i:m)) for v in l:j) - (1-x[i]+x[k])*(y[j]-y[l])<=z)
@constraint(Sparse, [i in 1:m, j in 1:m, k in 1:i, l in 0:j], -1/m* sum( (sum(a[u,v] for u in 1:k-1)+sum(a[u,v] for u in i+1:m)) for v in l+1:j-1) + (1-x[i]+x[k])*(y[j]-y[l])<=z)

@constraint(Sparse, [i in 1:m, j in 1:m, k in 0:i, l in 1:j-1], 1/m* sum( (sum(a[u,v] for v in 1:l)+sum(a[u,v] for v in j:m)) for u in k:i) - (x[i]-x[k])*(1-y[j]+y[l])<=z)
@constraint(Sparse, [i in 1:m, j in 1:m, k in 0:i, l in 1:j], -1/m* sum( (sum(a[u,v] for v in 1:l-1)+sum(a[u,v] for v in j+1:m)) for u in k+1:i-1) + (x[i]-x[k])*(1-y[j]+y[l])<=z)

@constraint(Sparse, [i in 1:m, j in 1:m, k in 1:i-1, l in 1:j-1], 1/m* ( sum(a[u,v] for v in 1:l, u in 1:k)+sum(a[u,v] for v in j:m, u in i:m))- (1-x[i])*(1-y[j])-y[l]*x[k]<=z)
...
output_file= open("/scratchbeta/clementf/PE_4.jl","w")
write(output_file, "x = ")
