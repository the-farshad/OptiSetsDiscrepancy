using JuMP
using Gurobi
log_file_path = "/scratchbeta/clementf/141.txt"
# 2d case, M5, double lattice 
...
@constraint(Sparse, [i in 2:m], x[2*i-3]+s-1 <=h[i])
@constraint(Sparse, [i in 2:m], x[2*i-3]+s>=h[i])
...
output_file= open("/scratchbeta/clementf/OC_141.jl", "w")
write(output_file, "x = ")
