...
@variable(Sparse, a[1:m+1,1:m+1,1:m+1], Bin)
...
# Bonus constraints
@constraint(Sparse, [i in 2:m], x[i]<=d+(i-1)/n)
@constraint(Sparse, [i in 2:m], y[i]<=d+(i-1)/n)
@constraint(Sparse, [i in 2:m], z[i]<=d+(i-1)/n)
@constraint(Sparse, [i in 2:m], x[i]>=i/n-d)
@constraint(Sparse, [i in 2:m], y[i]>=i/n-d)
@constraint(Sparse, [i in 2:m], z[i]>=i/n-d)
